...
<?php endif; ?>
  ...
  <div class="container py-5">
    <div class="row g-4 product-section">
      ...
      <div class="col-lg-6 text-center">
        <img src="<?php echo e(asset('storage/' . $product->image)); ?>" class="product-image img-fluid" alt="<?php echo e($product->name); ?>">
      </div>
      ...
      <div class="col-lg-6">
        ...
        <p class="mb-3">
          <span class="badge bg-dark"><?php echo e(optional($product->category)->name); ?></span>
          <span class="badge bg-secondary"><?php echo e(optional($product->subcategory)->name); ?></span>
          <span class="badge bg-success">Stock: <?php echo e($product->stock); ?></span>
        </p>

        <p class="price">
          <?php if($product->discount > 0): ?>
            ₹<?php echo e(number_format($product->price * (1 - $product->discount / 100), 2)); ?>

            <span class="old-price">₹<?php echo e(number_format($product->price, 2)); ?></span>
            <span class="discount"><?php echo e($product->discount); ?>% off</span>
          <?php else: ?>
        ...
        <p class="text-muted">Delivery:
          <?php echo e($product->delivery_charge ? '₹' . number_format($product->delivery_charge, 2) : 'Free'); ?>
        ...
        <form method="POST" action="<?php echo e(route('cart.add')); ?>">
          ...
          <div class="d-flex align-items-center mb-3">
            ...
            <input type="number" id="cartQty" name="quantity" value="1" min="1" max="<?php echo e($product->stock); ?>"
              class="form-control mx-2 text-center rounded-pill" style="max-width:80px;">
            <button type="button" class="btn btn-dark rounded-circle"
              onclick="var q=document.getElementById('cartQty');if(q.value<<?php echo e($product->stock); ?>)q.value++">+</button>
          </div>

          <div class="d-flex gap-2">
            <button type="submit" class="btn btn-gold flex-fill"><i class="bi bi-cart-fill"></i> Add to Cart</button>
        </form>
        <form method="POST" action="<?php echo e(route('wishlist.toggle')); ?>" class="flex-fill" id="wishlist-form">
          <?php echo csrf_field(); ?>
          <input type="hidden" name="product_id" value="<?php echo e($product->id); ?>">
          <button type="submit" class="btn btn-outline-dark w-100" id="wishlist-btn">
            <i class="bi bi-heart<?php echo e($product->isWishlistedBy(auth()->user()) ? '-fill text-danger' : ''); ?>"></i>
            Wishlist
          </button>
        </form>
      </div>
  ...
  <div class="tab-content mt-3">
    ...
    <div class="tab-pane fade" id="store-info">
      <?php if($seller): ?>
        <h4><?php echo e($seller->store_name ?? 'N/A'); ?></h4>
        <p><strong>Shop_Name:</strong> <?php echo e($seller->store_name ?? 'N/A'); ?></p>
        <p><strong>Address:</strong> <?php echo e($seller->store_address ?? 'N/A'); ?></p>
        <p><strong>Contact:</strong> <?php echo e($seller->store_contact ?? 'N/A'); ?></p>
        <a href="<?php echo e(route('store.products', $seller->id)); ?>" class="btn btn-outline-dark">View Store Products</a>
      <?php else: ?>
    ...
    <div class="tab-pane fade" id="reviews">
      ...
      <?php if(auth()->guard()->check()): ?>
        <form method="POST" action="<?php echo e(route('product.addReview', $product->id)); ?>" class="mb-4 p-3 rounded review-card">
          <?php echo csrf_field(); ?>
          <label class="form-label fw-bold">Your Rating</label>
          <div class="rating-stars mb-2 d-flex gap-1">
            <?php for($i = 5; $i >= 1; $i--): ?>
              <input type="radio" name="rating" value="<?php echo e($i); ?>" id="star<?php echo e($i); ?>">
              <label for="star<?php echo e($i); ?>" style="cursor:pointer; font-size:1.5rem;">★</label>
            <?php endfor; ?>
          </div>
        ...
        <?php $__currentLoopData = $reviews; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $review): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <div class="review-card mb-3 p-3">
            <div class="d-flex justify-content-between align-items-center mb-2">
              <span class="review-user"><?php echo e($review->user->name); ?></span>
            ...
            <small class="text-muted"><?php echo e($review->created_at->format('M j, Y')); ?></small>
          </div>
  ...
  <div class="other-products mt-5">
    ...
    <div class="row g-3">
      <?php $__empty_1 = true; $__currentLoopData = $otherProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $op): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <div class="col-6 col-md-3">
          <a href="<?php echo e(route('product.details', $op->id)); ?>" class="card h-100 text-decoration-none text-dark">
            <img src="<?php echo e(asset('storage/' . $op->image)); ?>" class="card-img-top" alt="<?php echo e($op->name); ?>">
            <div class="card-body text-center">
              <h6><?php echo e($op->name); ?></h6>
              <?php if($op->discount > 0): ?>
                <div class="text-gold fw-bold">₹<?php echo e(number_format($op->price * (1 - $op->discount / 100), 2)); ?></div>
                <small class="text-muted text-decoration-line-through">₹<?php echo e(number_format($op->price, 2)); ?></small>
                <small class="text-danger">(<?php echo e($op->discount); ?>% off)</small>
              <?php else: ?>
                <div class="text-gold fw-bold">₹<?php echo e(number_format($op->price, 2)); ?></div>
              <?php endif; ?>
            </div>
          </a>
        </div>
  ...
  <script>
    ...
    function shareOnWhatsApp() {
      ...
      const text = `Check out this amazing product: <?php echo e($product->name); ?> - ₹<?php echo e($product->price); ?> on grabbasket!`;
    ...
    function shareViaEmail() {
      const url = window.location.href;
      const subject = `Check out this product: <?php echo e($product->name); ?>`;
...
</html><?php /**PATH E:\e-com_updated_final\e-com_updated\resources\views/buyer/product-details.blade.php ENDPATH**/ ?>
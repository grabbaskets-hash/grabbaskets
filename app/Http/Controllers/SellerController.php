...
public function storeCategorySubcategory(Request $request)
{
    ...
    public function showSimpleUpload()
    {
        $sellerId = Auth::id();
        
        // Get statistics
        $totalProducts = Product::where('seller_id', $sellerId)->count();
        $productsWithImages = Product::where('seller_id', $sellerId)
            ->whereNotNull('image')
            ->where('image', '!=', '')
            ->count();
        $productsNeedingImages = $totalProducts - $productsWithImages;
        
        // Recently uploaded (products with images updated in last 24 hours)
        $recentlyUploaded = Product::where('seller_id', $sellerId)
            ->whereNotNull('image')
            ->where('image', '!=', '')
        ...
        $productsStillNeedingImages = Product::where('seller_id', $sellerId)
            ->where(function($query) {
        ...
        return view('seller.simple-image-upload', compact(
            'totalProducts', 
            'productsWithImages', 
            'productsNeedingImages', 
            'recentlyUploaded', 
            'productsStillNeedingImages'
        ));
    }

    public function processSimpleUpload(Request $request)
    {
        $request->validate([
        ...
        try {
            ...
            $tempDir = storage_path('app/temp/simple_upload_' . time());
            mkdir($tempDir, 0755, true);
            ...
            // Find images and match by unique ID
            $imageFiles = glob($tempDir . '/*.{jpg,jpeg,png,gif,webp}', GLOB_BRACE);
            $uploadedCount = 0;
            $matchedProducts = [];
            ...
            foreach ($imageFiles as $imagePath) {
                ...
                // Try to match by unique_id (e.g., SRM721)
                $product = Product::where('seller_id', $sellerId)
                ...
                if ($product) {
                    ...
                    if ($uploaded) {
                        ...
                        $uploadedCount++;
                        $matchedProducts[] = $product->unique_id;
                    }
                }
            }
            ...
            if ($uploadedCount > 0) {
                $message = "✅ Successfully uploaded {$uploadedCount} images! Products updated: " . implode(', ', $matchedProducts);
                return redirect()->route('seller.simpleUpload')->with('success', $message);
            } else {
                return redirect()->back()->with('error', 'No images could be matched. Make sure filenames match your product unique IDs (e.g., SRM721.jpg)');
            }
            
        } catch (\Exception $e) {
            Log::error('Simple upload failed: ' . $e->getMessage());
            return redirect()->back()->with('error', 'Upload failed: ' . $e->getMessage());
        }
    }
}

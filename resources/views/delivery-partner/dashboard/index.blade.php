        ...
        <div class="card">
            ...
            <div class="card-body">
                @if($availableOrders && $availableOrders->count() > 0)
                    @foreach($availableOrders->take(3) as $order)
                        <div class="order-card" onclick="viewOrder({{ $order->id }})">
                            <div class="order-header">
                                ...
                                <div class="text-end">
                                    <div class="fw-bold">₹{{ number_format($order->total_amount ?? 0, 0) }}</div>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <small class="text-muted">
                                    ...
                                    {{ Str::limit($order->delivery_address ?? 'Address not available', 30) }}
                                </small>
                                <button class="btn btn-primary btn-sm" onclick="acceptOrder({{ $order->id }}); event.stopPropagation();">
                                    Accept
                                </button>
                            </div>
                        </div>
                    ...
                    @foreach($recentOrders->take(3) as $order)
                        <div class="order-card" onclick="viewOrder({{ $order->id }})">
                            <div class="order-header">
                                ...
                                <div class="text-end">
                                    <span class="order-status status-{{ $order->delivery_status ?? 'pending' }}">
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <small class="text-muted">
                                    <i class="fas fa-rupee-sign me-1"></i>₹{{ number_format($order->total_amount ?? 0, 0) }}
                                </small>
                                <small class="text-muted">{{ $order->created_at->diffForHumans() }}</small>
                            </div>
                        </div>
...
<script>
    ...
    function viewOrder(orderId) {
        window.location.href = `/delivery-partner/orders/${orderId}`;
    }
